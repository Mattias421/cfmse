...
if __name__ == "__main__":
    ...
    for noisy_file in tqdm(noisy_files):
        ...
        n = y - x
        x_hat_16k = (
            librosa.resample(x_hat, orig_sr=sr_x_hat, target_sr=16000)
            if sr_x_hat != 16000
            else x_hat
        )
        x_16k = (
            librosa.resample(x, orig_sr=sr_x, target_sr=16000) if sr_x != 16000 else x
        )
    ...
    log.write(
        "SI-SDR: {:.1f} ± {:.2f}".format(*mean_std(df["si_sdr"].to_numpy())) + "\n"
    )
    log.write(
        "SI-SIR: {:.1f} ± {:.1f}".format(*mean_std(df["si_sir"].to_numpy())) + "\n"
    )
    log.write(
        "SI-SAR: {:.1f} ± {:.1f}".format(*mean_std(df["si_sar"].to_numpy())) + "\n"
    )
