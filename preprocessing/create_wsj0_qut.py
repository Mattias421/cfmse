...
if __name__ == "__main__":
    ...
    cafe, sr_QUT = load(
        glob(args.qut + "**/CAFE-CAFE-1.wav", recursive=True)[0], sr=None
    )
    car, sr_QUT = load(
        glob(args.qut + "**/CAR-WINDOWNB-1.wav", recursive=True)[0], sr=None
    )
    home, sr_QUT = load(
        glob(args.qut + "**/HOME-KITCHEN-1.wav", recursive=True)[0], sr=None
    )
    street, sr_QUT = load(
        glob(args.qut + "**/STREET-CITY-1.wav", recursive=True)[0], sr=None
    )
